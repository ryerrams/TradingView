...
def render(args):
    ...
    def draw(frame):
        ...
        if args.signals and on_va is not None and now >= rth_open and len(fired) < args.max_signals:
            ...
            side = None
            if poc_v > 0 and fast and not recent:
            ...
            if side:
                ...
                print(f"  {side:5s} {now:%a %H:%M} @ {px:,.2f}  "
                      f"row vol {totals[row] / poc_v:.0%} of POC, "
