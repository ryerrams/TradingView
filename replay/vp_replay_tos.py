...
def atr_on_timeframe(df, minutes, n=14):
    """ATR is bar-size dependent, so resample to a fixed timeframe first and
    map the value of the last COMPLETED bar back onto every row. Without this,
    1 minute input gives a tolerance roughly a third of what a 5 minute chart
    ...
    a = pd.Series(atr_wilder(r.reset_index(), n), index=r.index).shift(1)
    return a.reindex(df["time"], method="ffill").bfill().to_numpy()
...
def split_warmup(df, session_hour):
    """If the csv carries bars from before the replayed session, hold them back.
    They are used to warm up ATR only, never drawn or profiled. Matching TOS
    matters here: its ATR has the whole loaded chart behind it, mine otherwise
    starts cold at the session open and reads high in the first hours."""
    t = df["time"]
    starts = [i for i in range(1, len(df))
              if t.iloc[i].hour == session_hour and t.iloc[i - 1].hour != session_hour]
    if not starts:
...
def confluence_signals(df, g, rows, args, warmup=None):
    """Port of the Daily/Hourly Tail Confluence study.

    Runs on bars resampled to --conf-timeframe, because the original evaluates
    once per chart bar and edge detection is sensitive to that. On 1 minute rows
    the condition flickers and onlyOnePerDay latches onto the first flicker,
    which a 30 minute chart would never have seen as a transition at all.
    """
    tf = args.conf_timeframe
    src = df if warmup is None or warmup.empty else pd.concat([warmup, df], ignore_index=True)
    n_skip = 0 if warmup is None or warmup.empty else \
        len(warmup.set_index("time").resample(f"{tf}min").agg({"close": "last"}).dropna())
    r = (src.set_index("time")
           .resample(f"{tf}min")
           .agg({"open": "first", "high": "max", "low": "min",
                 "close": "last", "volume": "sum"})
           .dropna()
           .reset_index())
    atr_full = atr_wilder(r, 14)
    r = r.iloc[n_skip:].reset_index(drop=True)          # warmup feeds ATR, nothing else
    atr = atr_full[n_skip:]
    t = r["time"]
    edges = g["edges"]
    j_lo = np.clip(np.searchsorted(edges, r["low"].to_numpy(), "right") - 1, 0, rows - 1)
    j_hi = np.clip(np.searchsorted(edges, r["high"].to_numpy(), "right") - 1, 0, rows - 1)
    per = r["volume"].to_numpy() / (j_hi - j_lo + 1)

    day0 = t.iloc[0].date()
    gs = (int(args.conf_start[:2]), int(args.conf_start[2:]))
    ge = (int(args.conf_end[:2]), int(args.conf_end[2:]))
    lo_a, hi_a, cl_a = (r["low"].to_numpy(float), r["high"].to_numpy(float),
                        r["close"].to_numpy(float))
    INF = float("inf")
    run_lo, run_hi = INF, -INF
    hour_lo, hour_hi = INF, -INF
    hL, hH = [np.nan] * 3, [np.nan] * 3
    prev_hour = None
    totals = np.zeros(rows)
    prev_raw = {"LONG": False, "SHORT": False}
    fired = {"LONG": False, "SHORT": False}
    out = []
    cnt_lo = np.zeros(len(r), int)
    cnt_hi = np.zeros(len(r), int)
    tol_s = np.zeros(len(r))
    ...
    for i in range(len(r)):
        ...
        hm = (t.iloc[i].hour, t.iloc[i].minute)
        active = (t.iloc[i].date() != day0) and (gs <= hm <= ge)

        if args.conf_trace and (i == 0 or t.iloc[i].hour != t.iloc[i - 1].hour):
            print(f"    [trace] {t.iloc[i]:%a %H:%M}  Day Lo/Hi {run_lo:,.2f}/{run_hi:,.2f}  "
                  f"This Hour Lo/Hi {hour_lo:,.2f}/{hour_hi:,.2f}  Tol {tol:.2f}  "
                  f"Near-Low {nl}/{args.conf_lookback}  Near-High {nh}/{args.conf_lookback}"
                  f"{'' if active else '   (outside window)'}")

        if not active:
            prev_raw = raw
            continue
        for sd in ("LONG", "SHORT"):
            # optional proximity gate. The study only requires close < dayHigh,
            # which is true nearly everywhere, so a signal can fire far from the
            # extreme it is meant to be rejecting.
            dist = (run_hi - cl_a[i]) if sd == "SHORT" else (cl_a[i] - run_lo)
            too_far = args.conf_max_distance > 0 and dist > args.conf_max_distance * tol
            if raw[sd] and not prev_raw[sd] and not too_far \
                    and not (args.conf_once and fired[sd]):
                fired[sd] = True
                poc = g["centers"][int(totals.argmax())] if totals.max() > 0 else np.nan
                out.append((t.iloc[i], float(cl_a[i]), sd,
                            nl if sd == "LONG" else nh, float(poc), float(dist)))
        prev_raw = raw

    # map the per-bar counts back onto the original rows for the header label
    idx = pd.Series(range(len(r)), index=t)
    back = idx.reindex(df["time"], method="ffill").fillna(0).astype(int).to_numpy()
    return out, cnt_lo[back], cnt_hi[back], tol_s[back]
...
def render(args):
    df = load(args)
    warmup, df = split_warmup(df, args.session_hour)
    if not warmup.empty:
        print(f"warmup: {len(warmup)} bars before the session, used for ATR only")
    ...
    if args.signals and args.signal_mode in ("confluence", "both"):
        conf, cnt_lo, cnt_hi, tol_s = confluence_signals(df, g, rows, args, warmup)
        mode = ("ATR x %.2f" % args.conf_atr_mult if args.conf_tolerance_mode == "atr"
                else "%.2f pts" % args.conf_tolerance_points)
        print(f"confluence: {args.conf_timeframe}m bars, window {args.conf_start}-"
              f"{args.conf_end}, tolerance {mode}, need {args.conf_min_hours} of "
              f"{args.conf_lookback} hours -> {len(conf)} signal(s)")
        for c_ts, c_px, c_sd, c_n, c_poc, c_d in conf:
            print(f"  {c_sd:5s} confluence {c_ts:%a %H:%M} @ {c_px:,.2f}  {c_n} hourly "
                  f"tails near the day {'low' if c_sd == 'LONG' else 'high'}, "
                  f"{c_d:,.1f} pts from it, POC {c_poc:,.2f}")
    conf_left = list(conf)
    ...
    def draw(frame):
        ...
        while conf_left and conf_left[0][0] <= now:
            ts, px_c, sd, nh, poc_c, _d = conf_left.pop(0)
...
def main():
    ...
    p.add_argument("--session-hour", type=int, default=17,
                   help="session open hour. Bars before the last occurrence of it are "
                        "treated as ATR warmup and are not replayed.")
    p.add_argument("--conf-start", default="0830", help="no signals before this, HHMM")
    p.add_argument("--conf-end", default="1500", help="no signals after this, HHMM")
    p.add_argument("--conf-timeframe", type=int, default=5,
                   help="bar size the confluence rules run on, in minutes. Set this to "
                        "your thinkorswim chart's timeframe. Drives the hour registers, "
                        "the ATR tolerance and edge detection together.")
    p.add_argument("--conf-trace", action="store_true",
                   help="print Day Lo/Hi, This Hour Lo/Hi, Tolerance and the near counts "
                        "each hour, to diff against the study's labels")
    ...
    p.add_argument("--conf-max-distance", type=float, default=0.0,
                   help="reject a signal further than this many tolerances from the "
                        "day extreme it is rejecting. 0 disables, matching the study.")
